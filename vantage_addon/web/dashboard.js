...
function pctCard(kind, iconName, title, sub, s, abstainLine) {
  const cls = kind === 'memory' ? 'memory' : 'perf';
  if (s.abstained) {
    return `<div class="card card--abstain">
      <div class="card__top"><div class="iconwrap i-abstain">${svg(iconName)}</div>
      ...
      ${bullets([abstainLine])}</div>`;
  }
  return `<div class="card card--${cls}">
    <div class="card__top"><div class="iconwrap i-${cls}">${svg(iconName)}</div>
      <div><div class="card__label">${esc(title)}</div><div class="card__sub">${esc(sub)}</div></div></div>
    <div class="metric metric--${cls}"><span class="metric__num">${Math.round(s.point * 100)}</span><span class="metric__unit">%</span></div>
    <div class="range">likely <b>${pct(s.low)}&ndash;${pct(s.high)}</b></div></div>`;
}
...
function readinessCard(s, labels, d) {
  if (s.abstained) {
    ...
    return `<div class="card card--abstain">
      <div class="card__top"><div class="iconwrap i-abstain">${svg('gauge')}</div>
      ...
      <p class="abstain__lead">You need:</p>
      ${bullets(items)}</div>`;
  }
  const subs = Object.entries(s.sections || {}).map(([k, b]) => {
    return `<div class="subrow"><div class="subrow__label">${esc(labels[k] || k)}</div>
      <div class="subrow__score"><span class="subrow__val">${Math.round(b.point)}</span><span class="subrow__range">likely ${Math.round(b.low)}&ndash;${Math.round(b.high)}</span></div></div>`;
  }).join('');
  return `<div class="card card--ready">
    <div class="card__top"><div class="iconwrap i-ready">${svg('gauge')}</div>
      <div><div class="card__label">Readiness</div><div class="card__sub">your projected section scores</div></div></div>
    <div class="subsections">${subs}</div></div>`;
}
...
function render() {
  ...
  <header class="masthead">
    ...
    <div class="masthead__row">
      ...
      <div class="toolbar">
        ...
        <button class="btn btn--primary" onclick="vpy('back')">${svg('back')} Back to Anki</button>
      </div>
    </div>
    <div class="chips">
  ...
  <section class="section">
    <div class="scores">
      ${pctCard('memory', 'brain', 'Memory', 'how well you remember your cards', d.memory, memAbstain)}
      ${pctCard('perf', 'target', 'Performance', 'how well you apply it to new questions', d.performance, perfAbstain)}
  ...
  <div class="footmeta"><span>Updated ${esc(d.updated)}</span></div>`;
}

// Bridge to the Anki add-on (no-op in standalone preview).
...
const MOCK = {
  ...
  memory: { abstained: false, point: 0.89, low: 0.87, high: 0.90, how_sure: 'medium', n: 72, reasons: [] },
